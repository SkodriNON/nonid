...
const ABI = [
  "function recoverPUP(uint256 capsuleId,bytes32 newPinProofHash,bytes32 newPukProofHash,bytes32 newPupProofHash,uint8 feeToken)"
]
...
export async function POST(
  ...
  try {
    ...
    const newPinProofHash =
      String(
        body.newPinProofHash ||
        body.pinProofHash ||
        ""
      )

    const newPukProofHash =
      String(
        body.newPukProofHash ||
        body.pukProofHash ||
        ""
      )

    const newPupProofHash =
      String(
        body.newPupProofHash ||
        body.pupProofHash ||
        ""
      )
